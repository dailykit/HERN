...
import { BackSpaceIcon, SkipPhoneNumberModalImage } from '../../../assets/icons'
import { useTranslation } from '../../../context'
import { isClient } from '../../../utils'
import ReactHtmlParser from 'react-html-parser'

export const PhoneNoScreen = ({ config, setCurrentPage }) => {
   ...
   useEffect(() => {
      if (
         ...
         localStorage.getItem('phone') !== '2222222222'
      ) {
   ...
   const handleScreenChange = (isProceed = true) => {
      if (isProceed) {
         isClient && number.length > 0 && localStorage.setItem('phone', number)
      } else {
   ...
   const restrictedNumber = ['2222222222']
   ...
   return (
      ...
      >
         <header className="hern-kiosk__phone-screen-header">
            <div tw="flex items-center w-full">
               <button
                  ...
                  onClick={() => setCurrentPage('fulfillmentPage')}
               >
                  <svg
                     width="42"
                     height="42"
                  ...
                  >
                     <rect width="42" height="42" rx="21" fill="#7124B4"></rect>
                     <path
            ...
            <div>
               <button
                  ...
                  className="hern-kiosk__phone-screen__skip-btn"
               >
                  {t('Skip')}
               </button>
            </div>
         ...
         <div
            tw="h-[calc(100vh - 160px)]"
            className="hern-kiosk__phone-number-drawer__content"
         >
            ...
            <div className="hern-kiosk__phone-number-drawer__number">
               ...
               <button
                  ...
                  disabled={
                     number.length < 10 || restrictedNumber.includes(number)
                  }
      ...
      >
         <div className="ant-modal-title">
            ...
            <div style={{ margin: '1rem auto', fontSize: '2.2rem' }}>
               {ReactHtmlParser(
                  config?.phoneNoScreenSettings?.noPhoneNoWarning
                     ?.fulfillmentPage?.value || 'Are you sure ?'
               )}
            </div>
         </div>
         <div
