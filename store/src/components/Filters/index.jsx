...
export default function FilterComp({ children, ...props }) {
  const {
    ModalContainer: FilterModalContainer,
    isShow: isFilterShow,
    show: showFilter,
    hide: hideFilter,
  } = useModal();
  ...
  const checkIfSelected = (key, value) => {
    ...
    return Boolean(
      selectedFilter.hasOwnProperty(key) && selectedFilter[key].includes(value)
    );
  };
  ...
  return (
    <Wrapper isFilterSidebarVisible={isFilterSidebarVisible} {...props}>
              ...
              <div className="sidebar-single-panel" key={index}>
                ...
                >
                  ...
                  {selectedPanels.some((el) => el.id === index) ? (
                    <ChevronUp size="20" color={theme.colors.textColor4} />
                  ) : (
                    <ChevronDown size="20" color={theme.colors.textColor4} />
                  )}
                </div>
                <div
                  className={
                      ...
                      {item?.options.map((option) => (
                        <label key={option?.id}>
                          <Input
                            ...
                            onChange={() =>
                              handleCheckboxClick(item?.title, option?.title)
                            }
                          />
                          <li>{option?.title}</li>
                        </label>
      ...
      <FilterModalContainer isShow={isFilterShow}>
        <Modal type="bottomDrawer" isOpen={isFilterShow} close={hideFilter}>
