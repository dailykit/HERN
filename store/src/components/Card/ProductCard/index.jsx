...
import { useAuth, useExperienceInfo, useProduct } from "../../../Providers";
...
export default function ProductCard({ cardDetails, ...props }) {
  ...
  return (
    <Card {...props}>
      <CardImage>
        <img src={assets?.images[0]?.split(" ")?.join("%20")} alt="card-img" />
      </CardImage>
      <CardBody>
        ...
        {discount > 0 && <div class="product-discount-tag">{discount} off</div>}
          ...
          <div className="product_add_wrap">
            ...
            {props.isAdded ? (
              <span className="icon_wrap_1">
                <CheckCircle size="20" color={theme.colors.textColor4} />
              </span>
            ) : (
              <span className="icon_wrap">
                <AddCircleIcon size="20" color={theme.colors.tertiaryColor} />
              </span>
            )}
          </div>
        </Flex>
