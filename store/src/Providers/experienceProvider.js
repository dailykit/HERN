  ...
  useQuery(EXPERIENCE_CLASS_INFO, {
    ...
    onCompleted: async ({ privateClassType, publicClassType }) => {
      ...
      const initialDefaultClass = experienceClasses.find(
        (expClass) =>
          !expClass.isBooked && !isExpired(expClass?.startTimeStamp, new Date())
      );
      ...
      if (experienceClasses.length) {
        initialDispatchData = {
          ...
          totalPrice: initialDefaultClass?.classTypeInfo?.minimumBookingAmount,
          pricePerPerson:
        ...
        initialPollData = {
          ...
          pollPriceBreakDown: initialDefaultClass?.classTypeInfo?.priceRanges,
