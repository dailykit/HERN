...
function MyPolls({ navigationMenuItems, parsedData = [], footerHtml = '' }) {
               ...
               <div className="dashboard-right-div">
                  <div id="myPolls-top-01">
                     {Boolean(parsedData.length) &&
                        ReactHtmlParser(
                           parsedData.find(fold => fold.id === 'myPolls-top-01')
                              ?.content
                        )}
                  </div>
                  ...
                  <div id="myPolls-bottom-01">
                     {Boolean(parsedData.length) &&
                        ReactHtmlParser(
                           parsedData.find(
                              fold => fold.id === 'myPolls-bottom-01'
                           )?.content
                        )}
                  </div>
               </div>
...
export default protectedRoute(MyPolls)
