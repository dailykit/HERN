      ...
      if (rsvpStepIndex === 1) {
        if (participantId) {
          ...
          await CART.update.mutation({
            variables: {
              cartId:
                decodedToken?.cartId || experienceBookingParticipant?.cartId,
              _set: {
          ...
          console.log({ cartItemObjectForKit, cartItemObjectForExperience });

          if (!Boolean(decodedToken?.cartId)) {
            await CART_ITEM.createMany.mutation({
              variables: {
                objects: [cartItemObjectForExperience, cartItemObjectForKit],
              },
            });
          }
