...
export default function SubmitResponse({
  ...
  const [
    createParticipantChoice,
    { loading: isCreatingParticipantChoice },
  ...
  const [
    deleteParticipantChoice,
    { loading: isDeletingParticipantChoice },
    ...
    if (name === "email") {
      if (!IsEmailValid(value)) {
        ...
        setResponseDetail((prev) => {
          return {
            ...prev,
            [name]: { ...prev[name], error: "Please Provide a valid input!" },
          };
        });
      }
    } else {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setSelectedOptions([...experienceBookingParticipant?.participantChoices]);
    }
  }, [experienceBookingParticipant]);
      ...
      <div className="modal-content-div">
        <div style={{ padding: "1rem" }}>
          <h3 className="response-sub-head">
            Enter your information below and we’ll send you and invite with the
            date and time that{" "}
            {decodedToken?.invitedBy?.name || decodedToken?.invitedBy?.email}{" "}
            selects!
          </h3>
          <Input
          ...
          />
          {responseDetail?.email?.error ? (
            <Error margin="8px 0 1rem 0">{responseDetail?.email?.error}</Error>
          ) : (
            <Spacer yAxis="0.5rem" />
          )}
          <Input
          ...
          />
          {responseDetail?.phone?.error && (
            <Error margin="8px 0 1rem 0">{responseDetail?.phone?.error}</Error>
          )}
        </div>
        <Spacer yAxis="1rem" />
