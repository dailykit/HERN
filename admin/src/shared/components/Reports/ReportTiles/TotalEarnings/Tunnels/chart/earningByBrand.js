...
export const EarningByBrandChart = ({
   earningByBrandData,
   earningByBrandCompareData,
}) => {
   const [isLoading, setIsLoading] = useState(true)
   const [chartData, setChartData] = useState([])
   const { brandShopDateState } = React.useContext(BrandShopDateContext)
   React.useEffect(() => {
      if (earningByBrandCompareData.length > 0) {
         //present and past data merging
         const brandDataWithCompare = earningByBrandData.map(location => {
            const foundData = earningByBrandCompareData.find(
               x => x.id == location.id
            )
         ...
         setChartData(brandDataWithCompare)
         setIsLoading(false)
      } else {
         setChartData(earningByBrandData)
         setIsLoading(false)
      }
   }, [earningByBrandCompareData, earningByBrandData])
   ...
   const CustomTooltip = ({ active, payload }) => {
      if (active && payload && payload.length) {
         return (
            ...
            >
               <Spacer size="3px" />
               <Text as="text3">Location: {payload[0].payload['title']}</Text>
               <Text as="text3">
                  Earning:{' '}
         ...
         <ResponsiveContainer width="100%" height="100%">
            ...
            >
               ...
               <XAxis
                  ...
                  ticks={earningByBrandData.map(x => x.title)}
