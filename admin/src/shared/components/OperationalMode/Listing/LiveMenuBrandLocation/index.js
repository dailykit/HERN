...
const LiveMenuProductTable = ({ brandDetail }) => {
   ...
   const resetHandler = product => {
      if (
         ...
         resetProduct({
            variables: {
               where: {
                  brandId: { _is_null: true },
                  brand_locationId: { _eq: product.brand_locationId },
   ...
   const handleRowSelection = ({ _row }) => {
      ...
      const lastPersistence = localStorage.getItem(
         'selected-rows-id_brand-location-manager_table'
      )
   ...
   const tableLoaded = () => {
      const productGroup = localStorage.getItem(
         'tabulator_brand_location_manager_product_groupBy'
      )
   ...
   return (
      <>
         <ActionBar
            title="Product"
            groupByKeyName="brand_location_manager_product"
            defaultIdName="brand-location-manager"
...
const LiveMenuProductOptionTable = ({ brandDetail }) => {
   ...
   const handleRowSelection = ({ _row }) => {
      const rowData = _row.getData()
      const lastPersistence = localStorage.getItem(
         'selected-rows-id_brand-manager-location-option-product_table'
      )
      const lastPersistenceParse =
         lastPersistence !== undefined &&
         lastPersistence !== null &&
   ...
   const tableLoaded = () => {
      const productGroup = localStorage.getItem(
         'tabulator_brand_location_manager_product_option_groupBy'
      )
      const productGroupParse =
         productGroup !== undefined &&
         productGroup !== null &&
   ...
   return (
      <>
         <ActionBar
            title="Product"
            groupByKeyName="brand_location_manager_product_option"
            groupByOptions={groupByOptions}
            selectedRows={selectedRows}
            handleGroupBy={handleGroupBy}
            openTunnel={openTunnel}
...
const AvailableToggleStatus = ({ cell, update, check }) => {
   ...
   const toggleStatus = ({ Available, brand_locationId, Id }) => {
   ...
   React.useEffect(() => {
      if (checkedAvailable !== cell.getData().isAvailable) {
         ...
         console.log('mutation data on available', cell.getData().id)
      }
   }, [checkedAvailable])
...
const PublishedToggleStatus = ({ cell, update, check }) => {
   const [checkedPublished, setCheckedPublished] = React.useState(
      cell.getData().isPublished
   )
   // console.log('toggle data', cell.getData().name, cell.getData())
   const toggleStatus = ({ Published, BrandId, Id, brand_locationId }) => {
      if (check === 'productOptionId') {
         console.log('product optiion id', cell.getData())
         update({
            variables: {
               objects: {
